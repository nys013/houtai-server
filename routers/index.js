...
// 获取用户信息的路由(根据cookie中的userid)--不需要是因为前台通过localStorage保存，并且在前台判断，就没有经过后台了
...
router.get('/manage/product/list', (req, res) => {
  ...
  pageNum = pageNum*1
  pageSize = pageSize*1
  // 利用mongoose的limit和skip实现分页
  ProductModel.countDocuments({},function (err,total) {
      // total = count 
      ProductModel.find({},{},{skip:pageSize * (pageNum-1) , limit:pageSize},function (err,list) {
          const pages = Math.ceil( total  / pageSize)
          res.send({status: 0, data: {pageSize,pageNum,total,list,pages} })
      })
  })
})
...
/*
得到指定数组的分页信息对象
这个分页通过传查询到的所有数据，pageNum和pageSize，自己计算，然后通过循环push进新数组，然后再返回相应的数据的
但是其实mongoose的一些方法limit、skip也是能够做到的，而且应该更加高效一些的
 */
